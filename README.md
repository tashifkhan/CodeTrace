...
- **Saved Public Profiles:** Optional Google sign-in lets users claim a username and save their stacked account configuration at a clean public URL like `/<username>`.
...
The base dashboard still works without auth. Supabase is only required for Google login, username claiming, saved configurations, and clean public profile URLs.
...
For local Supabase development, also provide Google OAuth secrets to the Supabase CLI environment:
